...
static ABCC_MemAllocUnion abcc_uFreeMsgStack[ ABCC_CFG_MAX_NUM_MSG_RESOURCES ] = {0};
...
void ABCC_MemCreatePool( IDL_CTRL_HDL hCtrl )
{
   UINT16 i;
   ...
   {
      if(abcc_uFreeMsgStack[ i ].psAllocMsg == NULL)
        abcc_uFreeMsgStack[ i ].psAllocMsg = (ABCC_MemAllocType*)malloc(sizeof(ABCC_MemAllocType));
      abcc_uFreeMsgStack[ i ].psAllocMsg->iMagicCookie = ABCC_MEM_MAGIC_COOKIE;
      abcc_uFreeMsgStack[ i ].psAllocMsg->iBufferStatus = ABCC_MEM_BUFSTAT_FREE;
   }
}

ABP_MsgType* ABCC_MemAlloc( IDL_CTRL_HDL hCtrl )
{
...
void ABCC_MemFree( IDL_CTRL_HDL hCtrl, ABP_MsgType** pxItem)
{
...
ABCC_MemBufferStatusType ABCC_MemGetBufferStatus( IDL_CTRL_HDL hCtrl, ABP_MsgType* psMsg )
{
...
void ABCC_MemSetBufferStatus( IDL_CTRL_HDL hCtrl, ABP_MsgType* psMsg,
