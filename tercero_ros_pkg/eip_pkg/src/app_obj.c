...
**------------------------------------------------------------------------------
*/
BOOL IsResetRequestAllowed( IDL_CTRL_HDL hCtrl, UINT8 bResetType )
{
...
static void ObjectCommand( IDL_CTRL_HDL hCtrl, ABP_MsgType* psNewMessage )
{
   ...
   case ABP_APP_CMD_RESET_REQUEST:
   {
      ...
      */
      if( !IsResetRequestAllowed( hCtrl, ABCC_GetMsgCmdExt1( psNewMessage ) ) )
      {
   ...
   case ABP_CMD_RESET:
   {
      ...
      ** Perform a reset.
      */
      APP_ProcResetRequest( hCtrl, ABCC_GetMsgCmdExt1( psNewMessage ) );
      ABP_SetMsgResponse( psNewMessage, 0 );
      break;
   }
...
void APP_ProcResetRequest( IDL_CTRL_HDL hCtrl, UINT8 bResetType )
{
   switch( bResetType )
   {
   case ABP_RESET_FACTORY_DEFAULT:
...
void APP_ProcessCmdMsg( IDL_CTRL_HDL hCtrl, ABP_MsgType* psNewMessage )
{
