...
**------------------------------------------------------------------------------
*/
static void InstanceCommand( IDL_CTRL_HDL hCtrl, ABP_MsgType* psNewMessage )
{
   UINT32 lCycleTimeNs;
   UINT16 iSyncMode;
   UINT8  bAnbState;
...
**------------------------------------------------------------------------------
*/
static void ObjectCommand( IDL_CTRL_HDL hCtrl, ABP_MsgType* psNewMessage )
{
   UINT16 iStrLength;
...
**------------------------------------------------------------------------------
*/
static void UpdateAppStatus( IDL_CTRL_HDL hCtrl )
{
...
UINT32 SYNC_GetCycleTime( IDL_CTRL_HDL hCtrl )
{
   UINT32 lCycleTimeNs;
...
UINT32 SYNC_GetInputCaptureTime( IDL_CTRL_HDL hCtrl )
{
...
SYNC_SyncModeType SYNC_GetMode( IDL_CTRL_HDL hCtrl )
{
   return( (SYNC_SyncModeType)sync_sInstance.iSyncMode );
}

UINT32 SYNC_GetOutputValidTime( IDL_CTRL_HDL hCtrl )
{
...
void SYNC_ProcessCmdMsg( IDL_CTRL_HDL hCtrl, ABP_MsgType* psNewMessage )
{
   if( ABCC_GetMsgInstance( psNewMessage ) == ABP_INST_OBJ )
   {
      ...
      */
      ObjectCommand( hCtrl, psNewMessage );
   }
   else
   {
      /*
      ...
      */
      InstanceCommand( hCtrl, psNewMessage );
   }

   ABCC_SendRespMsg( hCtrl, psNewMessage );
}

void SYNC_SetInputProcessingTime( IDL_CTRL_HDL hCtrl, UINT32 lInputProcTimeNs )
{
   ABCC_PORT_UseCritical(hCtrl);
...
void SYNC_SetMinCycleTime( IDL_CTRL_HDL hCtrl, UINT32 lMinCycleTimeNs )
{
   ABCC_PORT_UseCritical(hCtrl);
...
void SYNC_SetOutputProcessingTime( IDL_CTRL_HDL hCtrl, UINT32 lOutputProcTimeNs )
{
   ABCC_PORT_UseCritical(hCtrl);
