...
void get_robot_state(const moveit_msgs::RobotTrajectory& m_traj, robot_state::RobotState& state,
                     std::vector<robot_state::RobotState>& vRobotStates)
{
  trajectory_msgs::JointTrajectory m_trajectory = m_traj.joint_trajectory;
  size_t node = m_trajectory.points.size();
...
void get_jacobian(std::vector<std::vector<double>>& vvjacobian, std::vector<robot_state::RobotState> vstates,
                  const robot_state::JointModelGroup* joint_model_group)
{
  ...
  for (auto state : vstates)
  {
    ...
    Eigen::Map<Eigen::MatrixXd>(&vjacobian[0], 6, 6) = jacobian;
...
void diff_angle(trajectory_msgs::JointTrajectory& traj, std::vector<double>& vdiff)
{
...
double calc_cost(trajectory_msgs::JointTrajectory& m_traj, std::vector<std::vector<double>>& vvjacobian)
{
  ...
  diff_angle(m_trajectory, vdiff);
  Eigen::MatrixXd Ediff = Eigen::Map<Eigen::MatrixXd>(&vdiff[0], vdiff.size() / 6, 6);
  ...
  for (int i = 0; i < Ediff.rows(); i++)
  {
    ...
    jacobi = Eigen::Map<Matrix6d>(&vjacobi[0], 6, 6);
