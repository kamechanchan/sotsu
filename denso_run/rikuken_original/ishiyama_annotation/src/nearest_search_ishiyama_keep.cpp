...
namespace nearest_point_extractor_ishiyama
{
    ...
    , the_number_of_object_(num)
    {
        ...
        pnh_->getParam("NearMesh_service_name", NearMesh_service_name_);
   ...
   void NearestPointExtractor::param_register(std::string sensor_topic, std::string mesh_topic, std::string output_topic, int num_of_near_point)
   {
    ...
    bool NearestPointExtractor::InputCallback(estimator::second_input::Request& req, estimator::second_input::Response& res)
    {   

        client_for_mesh_ = nh_.serviceClient<estimator::nearest_mesh>(NearMesh_service_name_);
        nearest_mesh_srv_.request.no = true;
        if (client_for_mesh_.call(nearest_mesh_srv_))
        {
            while (true)
            {
                try
            ...
            pcl_ros::transformPointCloud("world", transform_, req.in_cloud, msg_transformed);
