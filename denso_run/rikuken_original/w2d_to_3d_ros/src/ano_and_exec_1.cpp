#include <w2d_to_3d_ros/ano_and_exec_1.hpp>
...
void Ano_and_Exec::parameter_set()
{
    ...
    kiriwake_sub_ = nh_.subscribe(oculuder_topic_name_, 10, &Ano_and_Exec::InputCallback, this);
}

void Ano_and_Exec::InputCallback(const color_cloud_bridge::object_kiriwakeConstPtr &kiri_mes)
{
    sensor_msgs::CameraInfo cinfo;
    sensor_msgs::Image image1;
    get_one_message(cinfo, camera_topic_name_, nh_, timespan_);
    get_one_message(image1, image_topic_name_, nh_, timespan_);
    color_cloud_bridge::object_kiriwake kiri = *kiri_mes;
...
void Ano_and_Exec::paramter_set_bara(color_cloud_bridge::object_kiriwake mes)
{
    std::cout << "occuluder size is " << mes.occuluder_object.size() << std::endl;
    for (int i = 0; i < mes.occuluder_object.size(); i++) {
        target_frames_.push_back(mes.occuluder_object[i]);
    }
}
