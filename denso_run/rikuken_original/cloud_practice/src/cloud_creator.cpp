...
public:
    CloudCreator(ros::NodeHandle &nh, const std::string &topic_name, const std::string &file_name)
    {
        ...
        file_path_ = ros::package::getPath("cloud_practice") + "/data/" + file_name;
    }
...
{
public:
    NotRotatedCloudCreator(ros::NodeHandle &nh, const std::string &topic_name, const std::string &filename)
        : CloudCreator(nh, topic_name, filename){}
    ...
    virtual void create()
    {
        for (size_t i = 0; i < cloud_pcl_.points.size(); ++i)
        {
            ...
            cloud_pcl_.points[i].intensity = 3 * rand() / (RAND_MAX + 1.0f);
        }
    }
};
...
public:
    ...
    virtual void create()
    {
        ...
        for (size_t i = 0; i < cloud_pcl_.points.size(); ++i)
        {
            ...
            cloud_pcl_.points[i].z = 0.5 * rand() / (RAND_MAX + 1.0f);
            cloud_pcl_.points[i].intensity = 0.5 * rand() / (RAND_MAX + 1.0f);
        }
    }
};
