#include <annotation_package/nearest_search_4.hpp>
#include <list>
...
namespace nearest_point_extractor
{
   ...
   void NearestPointExtractor::param_register(std::string sensor_topic, std::string mesh_topic, std::string output_topic, int num_of_near_point)
   {
    ...
    {   
        ...
        bool record_ok, record_first;
        nh_.getParam("/record_is_ok", record_ok);
        nh_.getParam("/record_first", record_first);
        if (!record_ok)
        {
            return;
        }
        ...
        //     ros::WallDuration(14).sleep();
        //     std::cout << "stop after" << std::endl;
        //     nh_.setParam("/record_first", false);
        // }
        
        //print_parameter("extract mae");
        extract_cloud(*output_cloud_);
    ...
    void NearestPointExtractor::extract_cloud(pcl::PointCloud<pcl::PointXYZRGB> &out)
    {
