...
using handmarker_meshcloud_pub::MeshCloudParam;
...
HandMarkerMeshCloudPub::HandMarkerMeshCloudPub(ros::NodeHandle& nh)
  : nh_(nh), downsampled_mesh_pointcloud_ptr_pcl_(new pcl::PointCloud<pcl::PointXYZ>())
{
  link_names_.push_back("tercero_cover_for_mesh.stl");
  frame_names_.push_back("/tercero_cover_for_mesh_center");
  const std::string relative_path = "/meshes/handmarker/" + link_names_.back();
  this->getMesh(ros::package::getPath("ar_marker_urdf") + relative_path);
  this->transformMesh();
  mesh_pointcloud_publisher_ = nh_.advertise<sensor_msgs::PointCloud2>("/hand_mesh_cloud", 1);
}

void HandMarkerMeshCloudPub::getMesh(const std::string dir_path)
{
...
void HandMarkerMeshCloudPub::transformMesh()
{
...
void HandMarkerMeshCloudPub::downSample(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
...
void HandMarkerMeshCloudPub::publishCloud()
{
