...
void YoloBridge::parameter_set()
{
    ...
    // sub_ = nh_.subscribe(camera_topic_name_, 10, callback, this);
...
void YoloBridge::major()
{
    ...
    get_one_message<color_cloud_bridge::yolo_bridge>(msg_data_, output_topic_name_, nh_, timespan_);
    ROS_INFO_STREAM("iijanai");
    pcl::fromROSMsg(cloud_msgs_, trans_cloud_);
    ROS_INFO_STREAM(msg_data_);
    cv_img_ptr = cv_bridge::toCvCopy(msg_data_.output_img, sensor_msgs::image_encodings::BGR8);
    ...
    for(int i=0; msg_data_.out_data.size(); i++){
