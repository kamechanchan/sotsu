...
#include <gen_dataset/cloud_common.h>

class CloudPlanarSegmenter : public CloudOperator
{
public:
    CloudPlanarSegmenter(ros::NodeHandle &nh) :
        cloud_segmented_pub_(nh.advertise<sensor_msgs::PointCloud2>("photoneo/cloud_segmented", 1)),
        cloud_without_segmented_pub_(nh.advertise<sensor_msgs::PointCloud2>("photoneo/cloud_without_segmented", 1)),
...
private:
    void segment(pcl::PointIndices::Ptr inliers)
    {
        ...
        segmentation.setDistanceThreshold(0.0015);
...
main(int argc, char **argv)
{
    ...
    CloudOperationHandler handler(nh, new CloudPlanarSegmenter(nh), "photoneo/cloud_downsampled");
