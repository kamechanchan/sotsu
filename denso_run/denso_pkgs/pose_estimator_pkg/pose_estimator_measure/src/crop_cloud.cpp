...
CropCloud::CropCloud(ros::NodeHandle nh, ros::NodeHandle n)
  ...
  , voxel_n_(n.param("division_number", 64))
...
void CropCloud::cropCallback(const sensor_msgs::PointCloud2::ConstPtr& source_pc)
{
  ...
  if (!(source_pc->header.frame_id == frame_id_))
  {
    ...
    //点群をsensor座標系からGrand_Truth座標系に変換
...
void CropCloud::getVoxelSize(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud)
{
...
void CropCloud::downsampleCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
...
void CropCloud::makeVoxel(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, float leaf)
{
